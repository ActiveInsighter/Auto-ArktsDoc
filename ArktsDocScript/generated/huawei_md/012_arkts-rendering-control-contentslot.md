...
- 注册上下树事件，并通过事件获取对应的Content对象。 ```typescript auto nodeContentEvent = [](ArkUI_NodeContentEvent *event) {  ArkUI_NodeContentHandle content = OH_ArkUI_NodeContentEvent_GetNodeContentHandle(event);  if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_ATTACH_TO_WINDOW) {  } else if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_DETACH_FROM_WINDOW) {  }; }; OH_ArkUI_NodeContent_RegisterCallback(nodeContentHandle_, nodeContentEvent); ```
- 添加子组件。 ```typescript ArkUI_NodeHandle component; component = CreateNodeHandle(); OH_ArkUI_NodeContent_AddNode(nodeContentHandle_, component); ```
- 插入子组件。 ```typescript size_t position = 0; ArkUI_NodeHandle component1 = CreateNodeHandle(); OH_ArkUI_NodeContent_InsertNode(nodeContentHandle_, component1, position); ```
- 删除子组件。 ```typescript OH_ArkUI_NodeContent_RemoveNode(nodeContentHandle_, component1); ```
- 设置自定义属性。 ```typescript void *userData = CreateUserData(); OH_ArkUI_NodeContent_SetUserData(nodeContentHandle_, userData); ```
...
struct Parent {
  ...
  build() {
    Column() {
      ...
      ContentSlot(this.nodeContent_1);
      ContentSlot(this.nodeContent_1);
      ContentSlot(this.nodeContent_2);
    }
  }
}
```
